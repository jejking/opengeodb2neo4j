...
/**
 * Class to parse the tab delimited data file for Germany into a list
 * of data beans representing the contents of the file for further 
 * processing.
 * 
 * @author jejking
 *
...
public class PlaceParser {
	...
	public PlaceBean(int id, String ags, String ascii, String name,
		double lat, double lon, String amt, String plzs,
		String vorwahl, int einwohner, double flaeche, String kz,
		String typ, int level, int of, String invalid) {
	    super();
	    this.id = id;
	    this.ags = ags;
	    this.ascii = ascii;
	...
	public int hashCode() {
	    ...
	    result = prime * result
		    + ((vorwahl == null) ? 0 : vorwahl.hashCode());
	    return result;
	}
	...
	public boolean equals(Object obj) {
	    ...
	    if (Double.doubleToLongBits(flaeche) != Double
		    .doubleToLongBits(other.flaeche)) {
		return false;
	    }
	    if (id != other.id) {
		return false;
	...
	public String toString() {
	    return "PlaceBean [id=" + id + ", ags=" + ags + ", ascii=" + ascii
		    + ", name=" + name + ", lat=" + lat + ", lon=" + lon
		    + ", amt=" + amt + ", plzs=" + plzs + ", vorwahl="
		    + vorwahl + ", einwohner=" + einwohner + ", flaeche="
		    + flaeche + ", kz=" + kz + ", typ=" + typ + ", level="
		    + level + ", of=" + of + ", invalid=" + invalid + "]";
	}
    ...
    private final String[] headers = {"id","ags","ascii","name","lat","lon",
	    			"amt","plzs","vorwahl","einwohner","flaeche",
	    			"kz","typ","level","of","invalid"};
    
    private CellProcessor[] getCellProcessors() {
	
	/*
	 * Note that due to the way in which the data is contructed we can
	 * only assume that the ID field will be there. This means that
	 * all other columns are marked Optional. In some cases we need
	 * to trim the string as some data has white space lurking around.
	 */
	return new CellProcessor[] {
	    new NotNull(new Trim(new ParseInt())), // id
	...
	return new TabSeparatedBeanReader<>(PlaceBean.class, getCellProcessors(), headers).readDataFromStream(stream, true);
    }
}
