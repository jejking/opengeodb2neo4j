...
/**
 * Simple class to parse the tab-delimited representation of a set of German <i>Postleitzahlen</i> (postal codes) as
 * published by <a href="http://opengeodb.org/">Open GeoDB</a>.
 * 
 * @author jejking
 */
public class PlzParser {
    ...
    /**
     * Dumb POJO representing a line of data from the tab-delimited file.
     */
    public static class PlzTabBean {
        ...
        public PlzTabBean(int id, String plz, double lon, double lat, String placeName) {
            super();
            this.id = id;
            this.plz = plz;
            this.lon = lon;
        ...
        public String toString() {
            return "PlzTabBean [id=" + id + ", plz=" + plz + ", lon=" + lon + ", lat=" + lat + ", placeName="
                    + placeName + "]";
        }
    ...
    private CellProcessor[] getProcessors() {
        CellProcessor[] processors = new CellProcessor[] { new NotNull(new ParseInt()), // id
                new NotNull(), // plz
    ...
    public List<PlzTabBean> readDataFromStream(InputStream stream) throws IOException {
        return new TabSeparatedBeanReader<>(PlzTabBean.class, getProcessors(), headers).readDataFromStream(stream,
                false);
    }
}
